...
from partitioner.chunking import add_chunking_strategy
from partitioner.cleaners.core import (
    clean_extra_whitespace_with_index_run,
    index_adjustment_after_clean_extra_whitespace,
)
from partitioner.documents.coordinates import PixelSpace, PointSpace
from partitioner.documents.elements import (
    CoordinatesMetadata,
    Element,
    ElementMetadata,
    ElementType,
...
from partitioner.errors import PageCountExceededError
from partitioner.file_utils.filetype import add_metadata_with_filetype
from partitioner.file_utils.model import FileType
from partitioner.logger import logger, trace_logger
from partitioner.nlp.patterns import PARAGRAPH_PATTERN
from partitioner.partition.common.common import (
...
from partitioner.partition.common.lang import (
    check_language_args,
    prepare_languages_for_tesseract,
)
from partitioner.partition.common.metadata import get_last_modified_date
from partitioner.partition.pdf_image.analysis.layout_dump import (
    ExtractedLayoutDumper,
    FinalLayoutDumper,
    ObjectDetectionLayoutDumper,
    OCRLayoutDumper,
)
from partitioner.partition.pdf_image.analysis.tools import save_analysis_artifiacts
from partitioner.partition.pdf_image.form_extraction import run_form_extraction
from partitioner.partition.pdf_image.pdf_image_utils import (
    check_element_types_to_extract,
    convert_pdf_to_images,
    save_elements,
)
from partitioner.partition.pdf_image.pdfminer_processing import (
...
from partitioner.partition.pdf_image.pdfminer_utils import (
    PDFMinerConfig,
    open_pdfminer_pages_generator,
    rect_to_bbox,
)
from partitioner.partition.strategies import determine_pdf_or_image_strategy, validate_strategy
from partitioner.partition.text import element_from_text
from partitioner.partition.utils.config import env_config
from partitioner.partition.utils.constants import (
    OCR_AGENT_TESSERACT,
    SORT_MODE_BASIC,
    SORT_MODE_DONT,
    SORT_MODE_XY_CUT,
...
from partitioner.partition.utils.sorting import coord_has_valid_points, sort_page_elements
from partitioner.patches.pdfminer import patch_psparser
from partitioner.utils import first, requires_dependencies
...
def partition_pdf(
    ...
    **kwargs: Any,
    ...
    ignore_page_numbers
        If True, attempts to detect and remove page numbers from the extracted text.
    """
...
def _partition_pdf_or_image_local(
    ...
    **kwargs: Any,
    ...
    from partitioner.partition.pdf_image.ocr import process_data_with_ocr, process_file_with_ocr
    from partitioner.partition.pdf_image.pdfminer_processing import (
...
def _partition_pdf_or_image_with_ocr_from_image(
    ...
    **kwargs: Any,
    ...
    from partitioner.partition.utils.ocr_models.ocr_interface import OCRAgent
...
def _filter_page_numbers(elements: list[Element]) -> list[Element]:
    """Filter out elements that appear to be page numbers using intelligent detection.
    
    Uses multiple heuristics:
    1. Pattern matching for common page number formats
    2. Position analysis (top/bottom margins)
    3. Length and content analysis
    4. Context analysis (surrounding elements)
    """
    def is_page_number(element: Element, elements: list[Element], idx: int) -> bool:
        ...
        page_patterns = [
            r"^\d+$",  # Simple numbers
            r"^-\s*\d+\s*-$",  # Numbers with dashes
            r"^Page\s+\d+$",  # "Page X" 
            r"^Page\s+\d+\s+of\s+\d+$",  # "Page X of Y"
            r"^[ivxlcdm]+$",  # Roman numerals
            r"^[IVXLCDM]+$",  # Uppercase Roman numerals
        ]
        
        matches_pattern = any(re.match(pattern, text, re.IGNORECASE) for pattern in page_patterns)
        if not matches_pattern:
            return False
            
        # For very short numbers, check position
        if len(text) <= 3 and text.isdigit():
            if hasattr(element, "metadata") and hasattr(element.metadata, "coordinates"):
                coords = element.metadata.coordinates
                if hasattr(coords, "points"):
                    # Check if element is at top or bottom margins
                    y_pos = coords.points[0][1]
                    page_height = max(point[1] for point in coords.points)
                    if page_height > 0:
                        rel_pos = y_pos / page_height
                        if rel_pos < 0.1 or rel_pos > 0.9:
                            return True
                            
        # For explicit page labels, we can be more confident
        ...
        footer_markers = ["copyright", "all rights reserved", "confidential"]
    ...
    for idx, element in enumerate(elements):
        if not is_page_number(element, elements, idx):
